...
function init(a) {
    ...
    if(ctx.auto_resize) $(window).resize(function() { update.apply(ctx, [-1, true]); });
}
...
function change(force_render, dont_load_data)
{
    ...
    if(!dont_load_data) data.load_data_timer = setTimeout( function() { data.load_data_timer = null; load_data.call(ctx); }, ctx.load_delay );
